...
const loadQueue = async () => {
  ...
  try {
    ...
    if (res.queue) {
      ...
      // 🔥 关键修复：只有在没有当前运行实验且没有轮询时，才初始化新实验
      if (!currentRunningExperiment.value && !pollTimer) {
        const running = experimentQueue.value.find(e => e.status === 'running')
        if (running) {
          console.log(`[Experiment] loadQueue 检测到运行中实验: ${running.name}, 启动 benchmark`)
          try {
            // 🔥 关键修复：先启动 benchmark，成功后再设置状态和轮询
            await startBenchmarkTest(running)
            isExecuting.value = true
            currentRunningExperiment.value = running
            ElMessage.success(`开始实验: ${running.name}`)
            startPolling()
          } catch (startError: any) {
            console.error('[Experiment] loadQueue 启动 benchmark 失败:', startError)
            ElMessage.error(`启动 ${running.name} 失败: ${startError.message || '未知错误'}`)
  ...
  pollTimer = window.setInterval(async () => {
    try {
      ...
      if (completedExperimentIds.has(currentExp.id)) {
        console.log(`[Experiment] 轮询: 实验 ${currentExp.id} 已处理过完成，跳过`)
        return
      }
      ...
      // 🔥 关键修复：再次检查当前实验是否已变更（竞态条件）
      if (currentRunningExperiment.value?.id !== currentExp.id) {
        console.log(`[Experiment] 轮询: 实验已变更，跳过更新 (${currentExp.id} -> ${currentRunningExperiment.value?.id})`)
        return
      }
      ...
      if (isCompleted) {
        console.log(`[Experiment] 实验完成检测: ${currentExp.name}, 进度: ${progress.current}/${progress.total}`)
        
        // 🔥 关键修复：立即标记为已处理，防止重复处理
        ...
        // 🔥 关键修复：先记录已完成的ID，然后清空当前实验
        const completedExpId = currentExp.id
        currentRunningExperiment.value = null
        
        // 刷新队列 - loadQueue 会自动检测到下一个 running 实验并初始化
        await loadQueue()
        
        // 🔥 关键修复：loadQueue 已经处理了下一个实验的启动，这里只需要检查是否全部完成
        if (queueStats.value.pending === 0 && !currentRunningExperiment.value) {
          // 全部完成
          isExecuting.value = false
          completedExperimentIds.clear()  // 清空已完成的记录
          ElMessage.success('所有实验已完成！')
          setTimeout(() => goToResults(), 1500)
        }
        // 如果还有 running 的实验，loadQueue 已经自动设置了 currentRunningExperiment 和轮询
