...
export const experimentApi = {
  ...
  // 新的实验队列管理
  createExperiments: (data: { experiments: any[] }) => api.post('/experiments/create', data),
  getQueue: () => api.get('/experiments/queue'),
  startExperiments: () => api.post('/experiments/start'),
  getCurrent: () => api.get('/experiments/current'),
  listExperiments: () => api.get('/experiments/list'),
  clearExperiments: () => api.delete('/experiments/clear')
}
...
export const benchmarkApi = {
  ...
  startTest: (data: { 
    ...
    random_seed?: number;
    use_rag?: boolean;  // RAG知识检索（true=启用知识库+RAG）
    use_expert_routing?: boolean;  // 专家路由（true=使用学科专家prompt）
