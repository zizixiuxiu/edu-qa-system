...
import { benchmarkApi, expertApi, experimentApi } from '../api'
...
const addSelectedToIteration = async () => {
  ...
  try {
    ...
    // 显示即时反馈
    let msg = `✅ 已将 ${res.added_count || wrongIds.length} 道错题加入知识库`
    if (res.already_in_queue > 0) {
      msg += `（${res.already_in_queue} 道已在知识库中）`
    }
    if (correctCount > 0) {
    ...
    wrongIds.forEach((id: number) => {
      addedToIterationIds.value.add(id)
      const row = recentResults.value.find((r: any) => r.id === id)
      if (row) row.is_in_knowledge_base = true
    })
...
const addSingleToIteration = async (row: any) => {
  try {
    ...
    // 更新行状态，立即显示"已学习"
    row.is_in_knowledge_base = true
    
    let msg = `✅ 已加入知识库`
    if (res.already_in_queue > 0) {
      msg = `已在知识库中`
    }
    ElMessage.success(msg + '，可前往「知识库管理」查看')
  } catch (error: any) {
...
const checkAndStartExperiment = async () => {
  // 等待2秒确保页面加载完成
  await new Promise(resolve => setTimeout(resolve, 2000))
  
  try {
    // 获取当前实验
    const res: any = await experimentApi.getCurrent()
    if (!res.success) {
      console.log('[Benchmark] 没有正在执行的实验')
      return
    }
    
    const experiment = res.experiment
    console.log('[Benchmark] 检测到实验:', experiment.name)
    
    // 检查是否已有测试在运行
    const status = await benchmarkApi.getProgress() as any
    if (status.status === 'running') {
      console.log('[Benchmark] 已有测试在运行，同步状态')
      testing.value = true
      progress.value = status
      startProgressPolling()
      return
    ...
    // 自动启动测试
    ElMessage.info(`开始执行实验: ${experiment.name}`)
    
    // 设置实验配置
    if (experiment.config.random_seed) {
      testMode.value = 'random'
    }
    if (experiment.config.subject) {
      testSubject.value = experiment.config.subject
    }
    ...
    await benchmarkApi.startTest({
      expert_id: selectedExpert.value === 'auto' ? null : selectedExpert.value,
      mode: experiment.config.random_seed ? 'random' : testMode.value,
      subject: experiment.config.subject || testSubject.value,
      year: experiment.config.year || testYear.value,
    ...
    ElMessage.success('实验测试已启动')
    startProgressPolling()
    
  } catch (error) {
    console.error('[Benchmark] 检查实验失败:', error)
  }
}
</script>
